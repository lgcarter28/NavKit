...
- The estimator refactor is only partial. `InjectionPolicy`, `ResetPolicy`, `MeasurementPolicy`, and `NoisePolicy` exist. `KalmanFilter` is constrained on `StateDefPolicy`, injection, reset, and measurement-model boundaries, and `Sensor` is constrained on noise-policy compatibility. `Navigator`, filter boundaries, sensor collections, propagation, and update policies still need concept coverage.
