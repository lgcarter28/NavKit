...
- Treat `auto` as prohibited by default. Use explicit concrete types for stored or reused Eigen expressions, math temporaries, state vectors, matrices, dimensions, frame/unit-bearing values, and other domain values where lifetime, allocation, dimensions, or meaning matter. `auto` is acceptable only in narrow obvious cases such as range-loop/iterator variables, lambda or closure types, unavoidable template abstraction, and immediately consumed expressions where the concrete type is irrelevant and cannot affect lifetime or readability.
- Do not hardcode derived mathematical/statistical constants when a standard library or project utility can compute them from the meaningful inputs. For example, compute chi-square thresholds from confidence and degrees of freedom with the appropriate statistics function instead of pasting numeric quantiles into the source. Named literals are fine for true domain constants; derived analysis values should remain traceable to their defining parameters.
...
- `StateDefPolicy`, `StateSpaceDefPolicy`, and `SegmentPolicy` exist. `State<StateDef>` and `StateCov<StateDef>` are fixed-size Eigen aliases, while `NominalState<StateSpace>`, `ErrorState<StateSpace>`, and `ErrorStateCov<StateSpace>` name the split-state INS usage. `DefaultInsStateDef` aggregates `DefaultInsNominalStateDef` with quaternion nominal attitude and `DefaultInsErrorStateDef` with 3D small-angle attitude error.
- The estimator/navigation policy refactor now covers the current filter, filter-correction, sensor-filter, sensor-collection, propagation, per-sensor update-policy, and tuple-wide navigator-update boundaries. `InjectionPolicy`, `ResetPolicy`, `MeasurementModelPolicy`, `NoisePolicy`, `FilterPolicy`, `FilterCorrectionPolicy`, `FilterSensorPolicy`, `SensorCollectionPolicy`, `UpdatePolicy`, `NavigatorUpdatePolicy`, and `PropagationPolicy` exist.
- `KalmanFilter` performs measurement updates, covariance propagation, optional per-model statistics, injection, and reset. The default injection is INS-specific; the default covariance reset is intentionally a no-op.
- `Navigator` owns the current orchestration boundary: typed IMU ingestion, fixed-capacity IMU/covariance-step buffering, strapdown integration, filter-owned covariance propagation, and asynchronous sensor measurement updates.
...
- Keep configuration ownership explicit. Compile-time product values belong in selected compile-time config slices, runtime scenario values belong in runtime JSON, and true reusable embedded-library defaults may live in `include/navkit/core` only when they are an intentional product-core contract. App-support code may parse, validate, translate, and orchestrate selected configuration, but it must not invent fallback tuning values, logging cadences, run names, simulator seeds, noise levels, or other product/scenario defaults spread through helper code.
- Compile-time config values must be immutable product facts. Use `static constexpr` for scalar, enum, size, ID, boolean, and literal-like values; use `inline static const` for non-literal fixed-size config objects such as Eigen matrices; do not expose mutable static config objects. Runtime JSON overrides should populate separate runtime-owned values and be selected at the owning boundary rather than mutating compile-time config objects.
...
- Keep `docs/ROADMAP.md` focused on the current active phase and its active passes. Once a pass is fully complete, and only then, move its completed record into the matching dedicated phase file under `docs/roadmap_details/`; do not leave completed pass detail cluttering the active roadmap. Unfinished bullets must remain in `docs/ROADMAP.md` or move to the appropriate future phase detail file, never into a completed-pass record.
- Keep roadmap pass numbers aligned between `docs/ROADMAP.md` and the matching `docs/roadmap_details/phase_*.md` file. Within a phase, pass numbers must be unique and monotonically increasing in logical order; when archiving a completed pass, renumber any remaining active follow-on passes so the active roadmap continues after the latest archived pass number.
