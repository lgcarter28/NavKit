...
$b$ & Body & Vehicle-fixed frame, conventionally centered at the nominal vehicle center of gravity.\\
...
\noindent The origin frame $o$ replaces earlier placeholder notation for the Navigation Reference Point. Sensor lever arms are written as $l_{os}^{o}$, meaning the position of sensor frame $s$ with respect to origin frame $o$, resolved in the origin frame $b$.
...
\end{equation}
where $\alpha$ is the object frame, $\beta$ is the reference frame with respect to which the object is described, and $\gamma$ is the resolving frame. The object frame and reference frame must be distinct for motion quantities. The resolving frame may be any convenient frame. For example,
\begin{align}
\mathbf{r}_{eo}^{e} &: \text{position of } o \text{ with respect to } e \text{ resolved in } e,\\
\mathbf{v}_{eo}^{e} &: \text{velocity of } o \text{ with respect to } e \text{ resolved in } e,\\
\boldsymbol{\omega}_{io}^{o} &: \text{angular rate of } o \text{ with respect to } i \text{ resolved in } o,\\
\mathbf{f}_{io}^{o} &: \text{specific force of } o \text{ with respect to } i \text{ resolved in } o.
\end{align}
...
A rotation vector represents attitude error as a three-dimensional vector whose direction is the rotation axis and whose magnitude is the rotation angle. For small errors, the skew-symmetric matrix $[\delta\boldsymbol{\theta}]_\times$ provides the first-order perturbation used in error-state filtering.
...
\end{equation}
means the time derivative of the same vector with respect to the rotating ECEF frame, resolved in ECEF coordinates. When no derivative-frame subscript is shown on a vector that is a time derivative, the resolving frame is the same as the time derivative frame. For example, $\dot{\rb}^e$ is shorthand for $[\dot{\rb}^e]_e$.
...
\end{equation}
Now differentiate again with respect to the inertial frame. Start by defining the inertial velocity resolved in ECEF coordinates as
\begin{align}
\mathbf{u}^e
&= \left[\dot{\rb}^{e}\right]_i = \mathbf{v}_{i}^{e}, \\
&= \vb^e + \boldsymbol\omega_{ie}^e \times \rb^e,
\end{align}
and perform the inertial differentiation by substituting into the transport theorem, \cref{eq:transport_theorem_rev2},
\begin{equation}
...
\left[\dot{\mathbf{u}}^e\right]_e
&=
\frac{d}{dt}\left[\vb^e + \boldsymbol\omega_{ie}^e \times \rb^e\right]_e \\
&=
\dot{\vb}^e + \dot{\boldsymbol\omega}_{ie}^e \times \rb^e + \boldsymbol\omega_{ie}^e \times \dot{\rb}^e, \\
&=
\ab^e + \dot{\boldsymbol\omega}_{ie}^e \times \rb^e + \boldsymbol\omega_{ie}^e \times \vb^e.
\end{align}
...
\begin{equation}
\left[\dot{\mathbf{u}}^e\right]_e
= \ab^e + \boldsymbol\omega_{ie}^e \times \vb^e.
\end{equation}
...
\begin{equation}
\ab_i^e
= \ab^e + 2\Omie\vb^e+\Omie\Omie\rb^e.
...
=
\Coe \fb_{io}^o + \boldsymbol\gamma^e(\rb_{eo}^e),
\label{eq:newton_specific_force_nrp}
\end{equation}
where $\boldsymbol\gamma^e$ is mass-attraction gravity and $\fb_{io}^o$ is specific force at the origin frame $o$, resolved in body coordinates. Equating \cref{eq:inertial_accel_ecef_matrix_rev2,eq:newton_specific_force_nrp} gives
\begin{equation}
\dot{\vb}_{eo}^e
=
\Coe \fb_{io}^o
...
For any point $s$ fixed in the body, with lever arm $l_{os}^o$ from the origin frame $o$ to the sensor, the position, velocity, and acceleration of that point are
\begin{align}
...
\end{align}
An offset IMU measures angular rate at the IMU mounting frame and specific force at the IMU proof-mass location. If the IMU lever arm is $l_{ou}^o$, then the specific force at the IMU is approximately
\begin{equation}
\fb_u^o
=
\fb_{io}^o
+\dot{\boldsymbol\omega}_{eb}^o\times l_{ou}^o
+\boldsymbol\omega_{eb}^o\times\left(\boldsymbol\omega_{eb}^o\times l_{ou}^o\right),
...
The DCM $\Coe$ maps body-frame components into ECEF components:
\begin{equation}
\mathbf{a}^e=\Coe\mathbf{a}^o.
\end{equation}
Consider a vector $\mathbf{a}$ fixed in the body frame, so $[\dot{\mathbf{a}}^o]_b=0$. Its time derivative with respect to ECEF, resolved in ECEF, is caused by the angular velocity of body relative to ECEF:
\begin{equation}
...
\end{equation}
But since $\mathbf{a}^e=\Coe\mathbf{a}^o$ and $\mathbf{a}^o$ is constant,
\begin{equation}
[\dot{\mathbf{a}}^e]_e=\dot{\Coe}\mathbf{a}^o.
\end{equation}
Substitute $\mathbf{a}^e=\Coe\mathbf{a}^o$ into \cref{eq:fixed_body_vector_ecef_derivative_rev2}:
\begin{equation}
\dot{\Coe}\mathbf{a}^o = \skew{\boldsymbol\omega_{eb}^e}\Coe\mathbf{a}^o.
\end{equation}
Because this holds for any fixed body-frame vector,
\begin{equation}
\dot{\Coe}=\skew{\boldsymbol\omega_{eb}^e}\Coe.
...
Thus
\begin{equation}
\dot{\Coe}=\skew{\boldsymbol\omega_{ib}^e}\Coe-\skew{\boldsymbol\omega_{ie}^e}\Coe.
\end{equation}
Using $\boldsymbol\omega_{ib}^e=\Coe\boldsymbol\omega_{ib}^o$ and the identity $\skew{\Coe\mathbf{x}}=\Coe\skew{\mathbf{x}}\Ceo$, we obtain
\begin{equation}
\skew{\boldsymbol\omega_{ib}^e}\Coe
=\Coe\skew{\boldsymbol\omega_{ib}^o}\Ceo\Coe
=\Coe\skew{\boldsymbol\omega_{ib}^o}.
\end{equation}
Therefore the body-to-ECEF DCM kinematics are
\begin{equation}
\dot{\Coe}=\Coe\skew{\boldsymbol\omega_{ib}^o}-\skew{\boldsymbol\omega_{ie}^e}\Coe.
\label{eq:dcm_kinematics_body_to_ecef_rev2}
\end{equation}
This is the form typically used for ECEF mechanization when the gyro provides $\boldsymbol\omega_{ib}^o$ \cite{groves2013,titterton2004}.
...
\begin{equation}
\Coe(\quat)=(q_0^2-\mathbf{q}_v^T\mathbf{q}_v)\I+2\mathbf{q}_v\mathbf{q}_v^T+2q_0\skew{\mathbf{q}_v}.
\end{equation}
...
For angular rate $\boldsymbol\omega_{ib}^o$, define the pure quaternion
\begin{equation}
\boldsymbol\omega_q=\begin{bmatrix}0\\\boldsymbol\omega_{ib}^o\end{bmatrix}.
\end{equation}
...
For an offset IMU, the accelerometer calibrated specific force must be corrected from the IMU point to the origin frame $o$ using \cref{eq:imu_specific_force_to_nrp}. If the calibrated IMU specific force is $\hat{\fb}_u^o$, then
\begin{equation}
\hat{\fb}_{io}^o
=
\hat{\fb}_u^o
