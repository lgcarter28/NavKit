...
\newcommand{\Ceo}{\mathbf{C}^{o}_{e}}
\newcommand{\Ceb}{\mathbf{C}^{b}_{e}}
\newcommand{\Cei}{\mathbf{C}^{i}_{e}}
\newcommand{\Cen}{\mathbf{C}^{n}_{e}}
\newcommand{\Cie}{\mathbf{C}^{e}_{i}}
\newcommand{\Cio}{\mathbf{C}^{o}_{i}}
\newcommand{\Cin}{\mathbf{C}^{n}_{i}}
\newcommand{\Cib}{\mathbf{C}^{b}_{i}}
\newcommand{\Cbe}{\mathbf{C}^{e}_{b}}
\newcommand{\Cbi}{\mathbf{C}^{i}_{b}}
\newcommand{\Cbo}{\mathbf{C}^{o}_{b}}
\newcommand{\Cbn}{\mathbf{C}^{n}_{b}}
\newcommand{\Coe}{\mathbf{C}^{e}_{o}}
\newcommand{\Coi}{\mathbf{C}^{i}_{o}}
\newcommand{\Cob}{\mathbf{C}^{b}_{o}}
\newcommand{\Con}{\mathbf{C}^{n}_{o}}
\newcommand{\Cne}{\mathbf{C}^{e}_{n}}
\newcommand{\Cni}{\mathbf{C}^{i}_{n}}
\newcommand{\Cno}{\mathbf{C}^{o}_{n}}
\newcommand{\Cnb}{\mathbf{C}^{b}_{n}}
...
\chapter{Coordinate Systems, Attitude Representation, Kinematics, Earth and Gravity Modeling}

\section{Reference Frame Definitions}

This document uses a compact frame notation inspired by Groves \cite{groves2013}. The reference frames used throughout the document are summarized in \cref{tab:reference_frames}.

\begin{table}[htbp]
\centering
\caption{Reference frame definitions used throughout this document.}
\label{tab:reference_frames}
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{p{0.10\textwidth}p{0.27\textwidth}p{0.53\textwidth}}
\toprule
\textbf{Letter} & \textbf{Reference Frame} & \textbf{Description}\\
\midrule
$i$ & Earth-Centered Inertial (ECI) & Non-accelerating, non-rotating inertial frame used as the conceptual reference for Newtonian dynamics.\\
$e$ & Earth-Centered Earth-Fixed (ECEF) & Earth-fixed frame that rotates with the Earth.\\
$n$ & North-East-Down (NED) & Local geographic frame oriented north, east, and down.\\
$b$ & Body & Vehicle-fixed geometric frame, conventionally centered at the nominal vehicle center of gravity.\\
$o$ & Origin (Navigation Reference Point) & Body-fixed navigation origin frame used as the mechanization reference point and to aggregate sensor lever arms.\\
$u$ & IMU & Sensor frame centered at the inertial measurement unit.\\
$s$ & Sensor & Generic aiding-sensor frame, such as a GNSS antenna, altimeter, camera, magnetometer, or speed sensor.\\
\bottomrule
\end{tabular}
\end{table}

The origin frame $o$ is the primary mechanization frame used in Chapters 1--3. Its origin is the navigation reference point, and its axes are fixed in the vehicle. The body frame $b$ may still be useful for aerodynamics, actuator geometry, or legacy vehicle definitions, but the inertial mechanization is written in terms of $o$:
\[
r_{eo}^{e}, \qquad
v_{eo}^{e}, \qquad
C_o^e, \qquad
...
f_{io}^{o}.
\]
Sensor lever arms are written as
\[
l_{os}^{o},
\]
meaning the position of sensor frame $s$ with respect to origin frame $o$, resolved in origin frame $o$.

\section{Kinematic Vector Convention}

All kinematic Cartesian vectors use the convention
\begin{equation}
\mathbf{x}_{\beta\alpha}^{\gamma},
\end{equation}
where $\alpha$ is the object frame, $\beta$ is the reference frame with respect to which the object is described, and $\gamma$ is the resolving frame. The object frame and reference frame must be distinct for motion quantities. The resolving frame may be any convenient frame.

Examples:
\begin{align}
\mathbf{r}_{eo}^{e} &: \text{position of frame } o \text{ with respect to frame } e \text{ resolved in frame } e,\\
\mathbf{v}_{eo}^{e} &: \text{velocity of frame } o \text{ with respect to frame } e \text{ resolved in frame } e,\\
\boldsymbol{\omega}_{io}^{o} &: \text{angular rate of frame } o \text{ with respect to frame } i \text{ resolved in frame } o,\\
\mathbf{f}_{io}^{o} &: \text{specific force of frame } o \text{ with respect to frame } i \text{ resolved in frame } o.
\end{align}

This convention is intentionally explicit. For instance, a shorthand such as $\mathbf{r}_{s}^{e}$ should be written as $\mathbf{r}_{es}^{e}$ when the vector denotes the position of sensor frame $s$ with respect to ECEF frame $e$, resolved in ECEF coordinates.

\section{Attitude Representation Overview}

Attitude represents the orientation of one frame relative to another. This document primarily uses direction cosine matrices and quaternions for propagation, and small rotation vectors for error-state perturbations. Euler angles are useful for interpretation, but they are not ideal for propagation because of singularities and order-dependent definitions.

\subsection{Euler Angles}

Euler angles parameterize orientation as an ordered sequence of three rotations. For aerospace applications, yaw, pitch, and roll are commonly used for interpretation. They are intuitive, but they suffer from singularities and should generally not be used as the propagated attitude state in a high-performance inertial navigator.

\subsection{Rotation Vectors and Small-Angle Perturbations}

A rotation vector represents attitude error as a three-dimensional vector whose direction is the rotation axis and whose magnitude is the rotation angle. For small errors, the skew-symmetric matrix $[\delta\boldsymbol{\theta}]_\times$ provides the first-order perturbation used in error-state filtering. To first order,
\begin{equation}
C(\delta\boldsymbol{\theta})
\approx
I + [\delta\boldsymbol{\theta}]_\times,
\end{equation}
or with the opposite perturbation convention,
\begin{equation}
C(\delta\boldsymbol{\theta})
\approx
I - [\delta\boldsymbol{\theta}]_\times.
\end{equation}
The sign depends on whether the attitude error is defined as a left or right multiplicative perturbation.

\section{Notation for Rotating-Frame Derivatives}

A major source of confusion in inertial-navigation derivations is the distinction between the physical vector, the frame in which its components are resolved, and the frame with respect to which its time derivative is taken. This document uses square brackets to explicitly identify the derivative frame:
\begin{equation}
\left[
\frac{d\mathbf{x}_{\beta\alpha}^{\gamma}}{dt}
\right]_{\delta}.
\end{equation}
This means the time derivative is taken with respect to frame $\delta$, while the vector components remain resolved in frame $\gamma$.

When no derivative-frame subscript is shown, the derivative is taken with respect to the same frame in which the vector is resolved:
\begin{equation}
\dot{\mathbf{x}}_{\beta\alpha}^{\gamma}
\triangleq
\left[
\frac{d\mathbf{x}_{\beta\alpha}^{\gamma}}{dt}
\right]_{\gamma}.
...
\right]_{i}
\end{equation}
is the inertial derivative of the same physical position vector, still resolved in ECEF coordinates.

\section{Transport Theorem}

The transport theorem relates the time derivative of a vector as seen from two different rotating frames. For any vector $\mathbf{x}_{\beta\alpha}^{\gamma}$ resolved in frame $\gamma$, the derivative with respect to frame $\delta$ is
\begin{equation}
\boxed{
\left[
\frac{d\mathbf{x}_{\beta\alpha}^{\gamma}}{dt}
...
\right]_{\gamma}
+
\boldsymbol{\omega}_{\delta\gamma}^{\gamma}
\times
\mathbf{x}_{\beta\alpha}^{\gamma}
}
\label{eq:transport_theorem_general}
\end{equation}
where $\boldsymbol{\omega}_{\delta\gamma}^{\gamma}$ is the angular rate of frame $\gamma$ with respect to frame $\delta$, resolved in frame $\gamma$.

The theorem is independent of the object and reference frames $(\alpha,\beta)$ in the vector subscript. It depends only on the resolving frame $\gamma$ and the derivative frame $\delta$.

For the common case of an ECEF-resolved vector differentiated with respect to inertial frame $i$,
\begin{equation}
\left[
\frac{d\mathbf{x}_{\beta\alpha}^{e}}{dt}
\right]_i
=
\left[
\frac{d\mathbf{x}_{\beta\alpha}^{e}}{dt}
...
\times
\mathbf{x}_{\beta\alpha}^{e}.
\label{eq:transport_theorem_ecef}
\end{equation}

\section{ECEF Position, Velocity, and Acceleration Kinematics}

Apply \cref{eq:transport_theorem_ecef} to the ECEF position vector of the origin frame:
\begin{equation}
\left[
\frac{d\mathbf{r}_{eo}^{e}}{dt}
\right]_{i}
...
\right]_{e},
\end{equation}
the inertial velocity of the origin frame, resolved in ECEF coordinates, is
\begin{equation}
\left[
\frac{d\mathbf{r}_{eo}^{e}}{dt}
\right]_{i}
...
\times
\mathbf{r}_{eo}^{e}.
\label{eq:inertial_velocity_from_ecef_velocity}
\end{equation}

Differentiate again with respect to inertial frame $i$:
\begin{equation}
\left[
\frac{d^2\mathbf{r}_{eo}^{e}}{dt^2}
\right]_i
=
\left[
\frac{d}{dt}
...
\right]_i.
\end{equation}
Using the transport theorem on each term,
\begin{align}
\left[
\frac{d\mathbf{v}_{eo}^{e}}{dt}
\right]_i
&=
\left[
\frac{d\mathbf{v}_{eo}^{e}}{dt}
...
\right).
\end{align}
Assuming Earth rate is constant in ECEF coordinates,
\begin{equation}
\left[
\frac{d}{dt}
\left(
...
\label{eq:ecef_acceleration_transport_full}
\end{equation}
This is the kinematic source of the Coriolis and centrifugal terms in the ECEF velocity equation.

\section{Specific Force, Gravity, and the ECEF Navigation Equations}
...
\right]_i
=
C_o^e\mathbf{f}_{io}^{o}
+
\boldsymbol{\gamma}^{e}\left(\mathbf{r}_{eo}^{e}\right)
}
\label{eq:newton_specific_force_origin}
\end{equation}
where $\boldsymbol{\gamma}^{e}$ is mass-attraction gravity, and $\mathbf{f}_{io}^{o}$ is the specific force of origin frame $o$ with respect to inertial frame $i$, resolved in origin frame $o$.

Equating \cref{eq:ecef_acceleration_transport_full,eq:newton_specific_force_origin} gives
\begin{equation}
\left[
\frac{d\mathbf{v}_{eo}^{e}}{dt}
\right]_e
...
\boldsymbol{\gamma}^{e}\left(\mathbf{r}_{eo}^{e}\right)
-
2\boldsymbol{\omega}_{ie}^{e}\times\mathbf{v}_{eo}^{e}
-
...
\mathbf{r}_{eo}^{e}
\right).
\label{eq:ecef_velocity_mass_attraction_origin}
\end{equation}
Define plumb-bob gravity as
\begin{equation}
\mathbf{g}^{e}\left(\mathbf{r}_{eo}^{e}\right)
\triangleq
\boldsymbol{\gamma}^{e}\left(\mathbf{r}_{eo}^{e}\right)
-
\boldsymbol{\omega}_{ie}^{e}\times
...
\mathbf{r}_{eo}^{e}
\right).
\label{eq:plumb_bob_gravity_origin}
\end{equation}
Then the ECEF velocity equation becomes
\begin{equation}
\boxed{
\dot{\mathbf{v}}_{eo}^{e}
=
...
\label{eq:ecef_velocity_plumb_bob_origin}
\end{equation}
where the dot denotes differentiation with respect to the resolving frame $e$.

The complete ECEF translational mechanization for the origin frame is
\begin{equation}
\boxed{
\dot{\mathbf{r}}_{eo}^{e}
=
...
For any sensor frame $s$ fixed in the vehicle, define the lever arm
\begin{equation}
\mathbf{l}_{os}^{o}
\end{equation}
as the position of sensor frame $s$ with respect to origin frame $o$, resolved in origin frame $o$.

The ECEF position of sensor $s$ is
\begin{equation}
\boxed{
\mathbf{r}_{es}^{e}
=
\mathbf{r}_{eo}^{e}
+
C_o^e\mathbf{l}_{os}^{o}.
}
\label{eq:sensor_position_lever_arm_origin}
\end{equation}

The ECEF velocity of sensor $s$ is
\begin{equation}
\boxed{
\mathbf{v}_{es}^{e}
=
...
C_o^e
\left(
\boldsymbol{\omega}_{eo}^{o}
\times
\mathbf{l}_{os}^{o}
\right).
}
\label{eq:sensor_velocity_lever_arm_origin}
\end{equation}
This velocity is relative to the rotating ECEF frame. For inertial acceleration and specific-force corrections, the relevant angular rate is the inertial angular rate $\boldsymbol{\omega}_{io}^{o}$.

The inertial acceleration of the sensor point is, neglecting gravity variation across the vehicle,
\begin{equation}
\boxed{
\left[
\frac{d^2\mathbf{r}_{es}^{e}}{dt^2}
\right]_i
=
\left[
\frac{d^2\mathbf{r}_{eo}^{e}}{dt^2}
...
C_o^e
\left(
\dot{\boldsymbol{\omega}}_{io}^{o}
\times
\mathbf{l}_{os}^{o}
+
\boldsymbol{\omega}_{io}^{o}
\times
\left(
\boldsymbol{\omega}_{io}^{o}
\times
...
\label{eq:sensor_acceleration_lever_arm_origin}
\end{equation}

Therefore, the specific force at an offset IMU frame $u$, resolved in the origin frame, is approximately
\begin{equation}
\boxed{
\mathbf{f}_{iu}^{o}
=
...
\label{eq:imu_specific_force_offset_origin}
\end{equation}
Thus, if the mechanization state is defined at the origin frame $o$, the origin-frame specific force used in \cref{eq:ecef_velocity_plumb_bob_origin} is recovered from the IMU location by
\begin{equation}
\boxed{
\mathbf{f}_{io}^{o}
=
...
\label{eq:imu_specific_force_to_origin}
\end{equation}
...
The DCM $C_o^e$ maps components resolved in the origin frame $o$ into components resolved in ECEF:
\begin{equation}
\mathbf{a}^{e}=C_o^e\mathbf{a}^{o}.
\end{equation}
For a vector fixed in the origin frame, $[\dot{\mathbf{a}}^o]_o=0$. Its derivative with respect to ECEF, resolved in ECEF, is caused by the angular velocity of the origin frame relative to ECEF:
\begin{equation}
\left[
\dot{\mathbf{a}}^e
\right]_e
=
\boldsymbol{\omega}_{eo}^{e}\times\mathbf{a}^{e}
=
[\boldsymbol{\omega}_{eo}^{e}]_\times\mathbf{a}^{e}.
\end{equation}
Since $\mathbf{a}^e=C_o^e\mathbf{a}^o$ and $\mathbf{a}^o$ is constant,
\begin{equation}
\left[
\dot{\mathbf{a}}^e
\right]_e
=
\dot{C}_o^e\mathbf{a}^{o}.
\end{equation}
Substituting $\mathbf{a}^e=C_o^e\mathbf{a}^o$,
\begin{equation}
\dot{C}_o^e\mathbf{a}^{o}
=
[\boldsymbol{\omega}_{eo}^{e}]_\times C_o^e\mathbf{a}^{o}.
\end{equation}
Because this holds for any origin-frame vector,
\begin{equation}
\dot{C}_o^e
=
[\boldsymbol{\omega}_{eo}^{e}]_\times C_o^e.
\label{eq:dcm_kinematics_origin_left}
\end{equation}
The relative angular rate satisfies
\begin{equation}
\boldsymbol{\omega}_{eo}^{e}
=
\boldsymbol{\omega}_{io}^{e}
-
\boldsymbol{\omega}_{ie}^{e}.
\end{equation}
Using $\boldsymbol{\omega}_{io}^{e}=C_o^e\boldsymbol{\omega}_{io}^{o}$ and the identity
\begin{equation}
[C_o^e\mathbf{x}^{o}]_\times
=
C_o^e[\mathbf{x}^{o}]_\times C_e^o,
\end{equation}
we obtain
\begin{equation}
\boxed{
\dot{C}_o^e
=
C_o^e[\boldsymbol{\omega}_{io}^{o}]_\times
-
[\boldsymbol{\omega}_{ie}^{e}]_\times C_o^e.
}
\label{eq:dcm_kinematics_origin_to_ecef}
\end{equation}
Equivalently, define
\begin{equation}
\boldsymbol{\omega}_{eo}^{o}
...
\dot{C}_o^e
=
C_o^e[\boldsymbol{\omega}_{eo}^{o}]_\times.
}
\label{eq:dcm_kinematics_origin_relative}
\end{equation}
This is the ECEF attitude mechanization written entirely in the origin-frame convention.
...
\end{bmatrix}.
\end{equation}
The DCM corresponding to a scalar-first quaternion mapping origin-frame components to ECEF components is
\begin{equation}
C_o^e(\mathbf{q})
=
(q_0^2-\mathbf{q}_v^T\mathbf{q}_v)I
+
2\mathbf{q}_v\mathbf{q}_v^T
+
2q_0[\mathbf{q}_v]_\times.
\end{equation}
Given a DCM $C$, one common non-singular branch when $q_0$ is not small is
\begin{align}
...
For ECEF attitude propagation, form the relative angular rate
\begin{equation}
\boldsymbol{\omega}_{eo}^{o}
=
\boldsymbol{\omega}_{io}^{o}
...
C_e^o\boldsymbol{\omega}_{ie}^{e}.
\end{equation}
Define the pure quaternion
\begin{equation}
\boldsymbol{\omega}_{q}
=
\begin{bmatrix}
0\\
\boldsymbol{\omega}_{eo}^{o}
\end{bmatrix}.
\end{equation}
With the scalar-first convention and right multiplication by the relative angular-rate quaternion, the quaternion propagation equation is
\begin{equation}
\boxed{
\dot{\mathbf{q}}_{o}^{e}
=
...
\label{eq:quat_origin_to_ecef}
\end{equation}
In practical code, the same physics is often implemented by composing an origin-frame incremental rotation from the gyro with an Earth-frame incremental rotation:
\begin{equation}
C_{o,k+1}^{e}
\approx
C_{e_k}^{e_{k+1}}
...
\caption{First-order unaided inertial error-growth intuition.}\\
\toprule
...
IMU lever-arm error $\delta l_{ou}$ & Wrong rotational acceleration compensation & none directly & $(\dot\omega+\omega^2)\delta l_{ou}\,t$ & $\frac{1}{2}(\dot\omega+\omega^2)\delta l_{ou}\,t^2$\\
\bottomrule
\end{longtable}
...
For an offset IMU, the accelerometer calibrated specific force must be corrected from the IMU point to the origin frame $o$ using \cref{eq:imu_specific_force_to_origin}. If the calibrated IMU specific force is $\hat{\fb}_u^o$, then
\begin{equation}
\hat{\fb}_{io}^o
=
\hat{\fb}_u^o
-\hat{\dot{\boldsymbol\omega}}_{eb}^o\times l_{ou}^o
-\hat{\boldsymbol\omega}_{eb}^o\times\left(\hat{\boldsymbol\omega}_{eb}^o\times l_{ou}^o\right).
\end{equation}
...
\dot\vb_{eo}^e &= \Coe\fb_{io}^o+\gb^e(\rb_{eo}^e)-2\Omie\vb_{eo}^e,\\
\dot\Coe &= \Coe\skew{\boldsymbol\omega_{ib}^o}-\Omie\Coe.
\end{align}
...
\Coe &\approx \left(\I-\skew{\delta\boldsymbol\theta}\right)\hat{\Coe}.
\end{align}
...
\approx
-\delta\bb_a
-\operatorname{diag}(\hat{\fb}^o)\delta\mathbf{s}_a
-\mathbf{M}_a(\hat{\fb}^o)\delta\mathbf{m}_a
+\mathbf{w}_a
+\delta\fb_{lever}^o,
\label{eq:specific_force_error_model_rev2}
\end{equation}
where $\mathbf{M}_a(\hat{\fb}^o)$ maps accelerometer misalignment states into equivalent specific-force error. Similarly, the angular-rate error is
\begin{equation}
\delta\boldsymbol\omega_{ib}^o
\approx
-\delta\bb_g
-\operatorname{diag}(\hat{\boldsymbol\omega}_{ib}^o)\delta\mathbf{s}_g
-\mathbf{M}_g(\hat{\boldsymbol\omega}_{ib}^o)\delta\mathbf{m}_g
+\mathbf{w}_g.
...
\begin{align}
\delta\dot\vb^e
&=\delta\left(\Coe\fb^o\right)+\delta\gb^e-2\Omie\delta\vb^e.
\end{align}
Using $\Coe\approx(\I-\skew{\delta\boldsymbol\theta})\hat{\Coe}$ and $\fb^o\approx\hat{\fb}^o+\delta\fb^o$,
\begin{align}
\Coe\fb^o
&\approx\left(\I-\skew{\delta\boldsymbol\theta}\right)\hat{\Coe}\left(\hat{\fb}^o+\delta\fb^o\right)\\
&\approx\hat{\Coe}\hat{\fb}^o+\hat{\Coe}\delta\fb^o-\skew{\delta\boldsymbol\theta}\hat{\Coe}\hat{\fb}^o.
\end{align}
Since $-\skew{\delta\boldsymbol\theta}\mathbf{a}=\skew{\mathbf{a}}\delta\boldsymbol\theta$, the perturbation is
\begin{equation}
\delta\left(\Coe\fb^o\right)
=\hat{\Coe}\delta\fb^o+\skew{\hat{\Coe}\hat{\fb}^o}\delta\boldsymbol\theta.
\end{equation}
...
\begin{equation}
\delta\dot\vb^e
=\mathbf{G}_r\delta\rb^e-2\Omie\delta\vb^e+\skew{\hat{\Coe}\hat{\fb}^o}\delta\boldsymbol\theta+\hat{\Coe}\delta\fb^o.
\label{eq:vel_error_dyn_rev2}
\end{equation}

For attitude, linearization of \cref{eq:dcm_kinematics_origin_to_ecef} gives the standard first-order form
\begin{equation}
\delta\dot{\boldsymbol\theta}
\approx
-\Omie\delta\boldsymbol\theta
-\hat{\Coe}\delta\boldsymbol\omega_{ib}^o,
...
\mathbf{G}_r & -2\Omie & \skew{\hat{\Coe}\hat{\fb}^o} & \zero & -\hat{\Coe}\\
\zero & \zero & -\Omie & \hat{\Coe} & \zero\\
...
=-\hat{\Coe}\operatorname{diag}(\hat{\fb}^o),
\end{equation}
...
=-\hat{\Coe}\operatorname{diag}(\hat{\boldsymbol\omega}_{ib}^o).
\end{equation}
...
Suppose a measurement depends on a vector $\mathbf{a}^e=\Coe\mathbf{a}^o$. With the attitude perturbation convention
\begin{equation}
\Coe\approx(\I-\skew{\delta\boldsymbol\theta})\hat{\Coe},
\end{equation}
then
\begin{align}
\mathbf{a}^e
&\approx(\I-\skew{\delta\boldsymbol\theta})\hat{\Coe}\mathbf{a}^o\\
...
For sensor $s$ mounted at lever arm $l_{os}^o$, predicted sensor position and velocity are
\begin{align}
\hat{\rb}_s^e &= \hat{\rb}_{eo}^e+\hat{\Coe}l_{os}^o,\\
\hat{\vb}_s^e &= \hat{\vb}_{eo}^e+\hat{\Coe}\left(\hat{\boldsymbol\omega}_{eb}^o\times l_{os}^o\right).
\end{align}
The first-order position perturbation is
\begin{equation}
\delta\rb_s^e
=\delta\rb_{eo}^e+\frac{\partial(\Coe l_{os}^o)}{\partial\delta\boldsymbol\theta}\delta\boldsymbol\theta+\hat{\Coe}\delta l_{os}^o.
\end{equation}
Using \cref{eq:attitude_partial_vector_e_rev2},
\begin{equation}
\delta\rb_s^e
=\delta\rb_{eo}^e+\skew{\hat{\Coe}l_{os}^o}\delta\boldsymbol\theta+\hat{\Coe}\delta l_{os}^o.
...
For a GNSS antenna at lever arm $l_{og}^o$, the predicted antenna position is
\begin{equation}
\hat{\rb}_g^e=\hat{\rb}_{eo}^e+\hat{\Coe}l_{og}^o.
\end{equation}
...
\I & \zero & \skew{\hat{\Coe}l_{og}^o} & \zero & \cdots & \hat{\Coe}
\end{bmatrix},
\end{equation}
...
\begin{equation}
\hat{\vb}_g^e=\hat{\vb}_{eo}^e+\hat{\Coe}\left(\hat{\boldsymbol\omega}_{eb}^o\times l_{og}^o\right).
\end{equation}
The velocity update residual is
\begin{equation}
\mathbf{r}_v=\mathbf{z}_{v,gps}^e-\hat{\vb}_g^e.
\end{equation}
The Jacobian includes direct velocity sensitivity, attitude sensitivity through the rotated lever-arm velocity term, gyro-error sensitivity through $\hat{\boldsymbol\omega}_{eb}^o$, and optional lever-arm sensitivity. In many systems the lever-arm velocity term is small, but for high angular-rate vehicles it can be essential.
...
\mathbf{u}_j^{eT} & \zero & \mathbf{u}_j^{eT}\skew{\hat{\Coe}l_{og}^o} & \cdots & c & 0
\end{bmatrix}.
\end{equation}
...
(1+\bar s_{alt})\mathbf{h}_r & \zero & (1+\bar s_{alt})\mathbf{h}_r\skew{\hat{\Coe}l_{os}^o} & \cdots & 1 & h(\hat{\rb}_s^e)
\end{bmatrix},
\end{equation}
...
\begin{equation}
\hat z=(1+s_v)\mathbf{e}_x^{sT}\mathbf{C}_b^s\mathbf{C}_e^o\hat{\vb}_s^e+b_v.
\end{equation}
where
\begin{equation}
\hat{\vb}_s^e=\hat{\vb}_{eo}^e+\hat{\Coe}(\hat{\boldsymbol\omega}_{eb}^o\times l_{os}^o).
\end{equation}
The direct velocity partial is
\begin{equation}
\frac{\partial z}{\partial\delta\vb^e}=(1+s_v)\mathbf{e}_x^{sT}\mathbf{C}_b^s\mathbf{C}_e^o.
\end{equation}
...
\begin{equation}
\hat{\mathbf{z}}_m=\mathbf{C}_b^m\mathbf{C}_e^o\mathbf{m}^e(\hat{\rb}_s^e)+\bb_m.
\end{equation}
...
\begin{align}
\rb_c^e &= \rb_{eo}^e+\Coe l_{oc}^o,\\
\mathbf{C}_e^c &= \mathbf{C}_b^c\Ceo.
\end{align}
...
Let $o$ denote the origin frame used as the mechanization reference. The nominal navigation state estimates the position, velocity, and attitude of the origin frame. Let sensor $s$ have body-frame lever arm $l_{os}^o$ from the origin frame $o$ to the sensor phase center or measurement point. The ECEF position of the sensor is
\begin{equation}
\rb_s^e = \rb_{eo}^e + \Coe l_{os}^o.
...
\begin{equation}
\vb_s^e = \vb_{eo}^e + \Coe\left(\boldsymbol\omega_{eb}^o \times l_{os}^o\right),
\label{eq:nrp_sensor_velocity}
\end{equation}
where $\boldsymbol\omega_{eb}^o$ is the angular rate of the body frame relative to ECEF, resolved in body coordinates. The corresponding acceleration is
\begin{equation}
\ab_s^e = \ab_{eo}^e + \Coe\left(\dot{\boldsymbol\omega}_{eb}^o \times l_{os}^o + \boldsymbol\omega_{eb}^o \times \left(\boldsymbol\omega_{eb}^o \times l_{os}^o\right)\right).
