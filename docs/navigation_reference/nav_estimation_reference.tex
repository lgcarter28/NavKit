...
\midrule
$i$ & Earth-Centered-Inertial Frame & Non-accelerating, non-rotating space (ECI).\\
$e$ & Earth-Centered-Earth-Fixed Frame & Rotates with the Earth (ECEF).\\
$n$ & NED Frame & Local geographic frame oriented to North-East-Down (NED).\\
$b$ & Body Frame & Rigidly attached to the vehicle, centered at the nominal CG.\\
$o$ & Origin Frame & Origin frame used to aggregate sensor lever arms.\\
$u$ & IMU Frame & Centered at the IMU.\\
$s$ & Generic Sensor Frame & Generic aiding-sensor frame.\\
\bottomrule
\end{tabular}
\end{center}

\noindent Throughout this document, $o$ denotes the arbitrary body-fixed origin frame used as the mechanization reference point. Sensor lever arms are written as $l_{os}^{b}$, meaning the vector from the origin frame $o$ to sensor frame $s$, resolved in the body frame $b$.


\chapter*{Preface}
This reference is written as an interview preparation ``answer sheet'' rather than as a conventional textbook. The derivations are intentionally interleaved with implementation notes and interview-style explanations so that each topic can be reviewed in one place without repeatedly jumping to appendices. The focus is practical navigation systems engineering: inertial mechanization, error-state filtering, delayed measurements, statistical consistency, observability, GPS-denied aiding, and embedded estimator architecture.
