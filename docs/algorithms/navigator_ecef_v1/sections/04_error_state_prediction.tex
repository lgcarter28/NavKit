...
\right)}
{\partial\pb_{eb}^e}
\right|_{\hat{\pb}_{eb}^{e}},\\
B_g &= \operatorname{diag}(\boldsymbol{\beta}_g),\\
B_a &= \operatorname{diag}(\boldsymbol{\beta}_a).
\end{align}
For Earth products, the selected WGS-84/J2-capable gravity policy supplies
the mass-attraction gradient $G_\gamma^e$, and the rotating-planet model adds
$-\Omega_{ie}^e\Omega_{ie}^e$ so that
$G_p^e=G_\gamma^e-\Omega_{ie}^e\Omega_{ie}^e$ is the complete
apparent-gravity gradient.
...
The v1 IMU error model separates white measurement noise from first-order
...
\mathbf{w}_{ba}.
\end{align}
Here $\boldsymbol{\beta}_g$ and $\boldsymbol{\beta}_a$ are per-axis bias
correlation rates in $\si{\per\second}$. Setting either rate vector to zero
recovers the random-walk model for that bias family.
...
\zero & \zero & \zero & -B_g & \zero\\
\zero & \zero & \zero & \zero & -B_a
\end{bmatrix},
...
accelerometer-bias driving-noise spectral densities. When the matching
correlation rate is zero, the driving-noise PSD is the random-walk PSD.
\begin{center}
\begin{tabular}{lll}
\toprule
Block & Meaning & Units \\
...
$S_{bg}$ & gyro-bias driving-noise PSD & $\si{\radian^2\per\second^3}$ \\
$S_{ba}$ & accelerometer-bias driving-noise PSD & $\si{\meter^2\per\second^5}$ \\
$\boldsymbol{\beta}_g$ & gyro-bias correlation rate & $\si{\per\second}$ \\
$\boldsymbol{\beta}_a$ & accelerometer-bias correlation rate & $\si{\per\second}$ \\
\bottomrule
\end{tabular}
\end{center}
...
With
\begin{align}
\bar C_{b,k}^{e} &= C_b^e\left(\hat{\quat}_{b,\mathrm{mid},k}^{e}\right)
\quad\text{from \cref{eq:v1_midpoint_attitude}},\\
\hat{\fb}_{ib,k}^{e} &= \bar C_{b,k}^{e}\hat{\fb}_{ib,k}^{b},
\qquad
\hat{\fb}_{ib,k}^{b}
=
...
\quad\text{from \cref{eq:v1_apparent_gravity_gradient}},\\
B_{g,k} &= \operatorname{diag}(\boldsymbol{\beta}_{g,k}),\\
B_{a,k} &= \operatorname{diag}(\boldsymbol{\beta}_{a,k}),
\end{align}
...
\begin{bmatrix}
\zero & \I & \zero & \zero & \zero\\
G_{p,k}^{e} & -2\Omega_{ie}^{e} & -\crossmat{\hat{\fb}_{ib,k}^{e}} & \zero & -\bar C_{b,k}^{e}\\
\zero & \zero & -\Omega_{ie}^{e} & -\bar C_{b,k}^{e} & \zero\\
\zero & \zero & \zero & -B_{g,k} & \zero\\
\zero & \zero & \zero & \zero & -B_{a,k}
\end{bmatrix}.
}
...
\begin{bmatrix}
\zero & \zero & \zero & \zero\\
\zero & \bar C_{b,k}^{e} & \zero & \zero\\
\bar C_{b,k}^{e} & \zero & \zero & \zero\\
\zero & \zero & \I & \zero\\
\zero & \zero & \zero & \I
\end{bmatrix}.
}
...
and the accelerometer-bias random-walk special case
($\boldsymbol{\beta}_a=\zero$) are considered in the
