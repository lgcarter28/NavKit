...
\subsection{Position Chi-Square Check}

The position observation instantiates
\cref{sec:v1_chi_square_innovation_acceptance} with $m_p=3$,
$S_p=H_pP^-H_p^T+R_p$, and
$\epsilon_p=\mathbf{r}_p^TS_p^{-1}\mathbf{r}_p$. Its independently configured
acceptance probability $p_{\mathrm{acc},p}$ determines the position threshold
$\gamma_p=F_{\chi^2_3}^{-1}(p_{\mathrm{acc},p})$.
...
\end{align}
The positive inner Earth-rate term follows from
$C_b^{e,true}\approx(\I+\crossmat{\delta\boldsymbol{\theta}})\hat C_b^e$:
the perturbation of $-C_e^b\omegab_{ie}^e$ is
$-\hat C_e^b\crossmat{\omegab_{ie}^e}\delta\boldsymbol{\theta}$, and crossing
that result with the lever arm introduces the second sign change. The
implementation obtains $\hat{\omegab}_{ib}^b$ from the latest valid IMU
increment and applies the nominal gyro-bias correction; simulator truth
angular rate is not passed into the filter observation context.
If this sign convention changes, the velocity lever-arm Jacobian must change
with it.

\subsection{Velocity Chi-Square Check}

The velocity observation independently instantiates
\cref{sec:v1_chi_square_innovation_acceptance} with $m_v=3$,
$S_v=H_vP^-H_v^T+R_v$, and
$\epsilon_v=\mathbf{r}_v^TS_v^{-1}\mathbf{r}_v$. Its configured probability
$p_{\mathrm{acc},v}$ determines
$\gamma_v=F_{\chi^2_3}^{-1}(p_{\mathrm{acc},v})$. Position acceptance does not
implicitly accept velocity, or vice versa, even when both observations share a
timestamp.
...
\subsection{Combined Position/Velocity Chi-Square Check}

When all six position and velocity components are processed as one joint
observation, the generic gate uses $m_{gnss}=6$ and the full joint innovation
covariance. Its threshold is therefore
$F_{\chi^2_6}^{-1}(p_{\mathrm{acc},gnss})$, not either 3-DOF family threshold.
If only position or only velocity is valid, the corresponding 3-DOF model and
gate must be used instead of treating missing components as a 6-DOF update.
