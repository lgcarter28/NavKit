...
The NavKit v1 convention is left-multiplicative correction of the whole nominal
body-to-ECEF quaternion:
\begin{equation}
\boxed{
\hat{\quat}_{b}^{e,+}
\leftarrow
...
\begin{warningbox}
The left-multiplication convention in \cref{eq:v1_quat_injection} fixes the
meaning of $\delta\boldsymbol{\theta}$ and matches
\cref{eq:v1_attitude_error_dcm_relation}. If implementation chooses right
multiplication instead, the prediction, measurement Jacobians, injection, reset
Jacobian, and tests must be updated together.
\end{warningbox}
