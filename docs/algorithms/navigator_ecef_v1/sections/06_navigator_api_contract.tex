...
The explicit \texttt{process\_measurements()} stage is not the normal client
entry point for the INS Navigator. It remains available for tests, profiling,
and advanced scheduling; ordinary clients should call \texttt{update()} once
the required timestamped data has been ingested.
...
EKF correction and acceptance & \cref{eq:v1_measurement_residual,eq:v1_innovation_covariance,eq:v1_kalman_gain,eq:v1_error_state_correction,eq:v1_joseph_covariance_update,eq:v1_nis,eq:v1_chi_square_gate_threshold,eq:v1_chi_square_gate_acceptance} & Measurement models own $\hat{\mathbf{z}}$, $H$, $R$, and the observation dimension that determines gate degrees of freedom. \\
...
GNSS aiding & \cref{eq:v1_gnss_pos_pred,eq:v1_gnss_pos_h,eq:v1_gnss_vel_pred,eq:v1_gnss_vel_h,eq:v1_gnss_vel_perturbation} & Includes antenna lever arm and separate position/velocity instantiations of the generic chi-square acceptance contract. \\
\bottomrule
\end{tabular}
\end{center}
