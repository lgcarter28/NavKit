...
- [x] Implement the first ECEF v1 propagation policy against the current state-vector reality: use quaternion math internally for attitude propagation, then write the propagated ECEF-to-body attitude back into the existing `Att`/RPY segment. Keep a future cleanup item for true nominal quaternion storage if the current `StateDef` shape becomes too constraining.
- [x] Keep the v1 `InsStateDef` configs aligned with the algorithm document by using the 15-state PVA+gyro-bias+accelerometer-bias layout. Leave online scale-factor/misalignment estimation for a deliberately designed future pass.
...
- [x] Implement the first real INS path from the `navigator_ecef_v1` equation-to-code map, using the Pass 5c IMU emulator as the primary data source:
  ...
  - [ ] Aiding integration: GNSS velocity observations with configured antenna lever arm and finite-difference Jacobian tests. GNSS position aiding remains supported through the existing configured sensor path.
...
## Pass 5e — Navigator timing, state-shape, and INS diagnosis cleanup

- [ ] Add explicit medium-rate covariance propagation configuration to the selected NavKit product configs. Start with a compile-time default of 100 Hz because this rate informs fixed-size STM/process-noise history capacity and future delayed-measurement support.
- [ ] Add a bounded covariance-step history buffer owned by `Navigator`, defaulting to 256 entries for the first implementation. Keep the current accumulated pending covariance step for the normal no-latency path, but retain enough recent `Phi`/`Q_d` history for inspection and future latent-measurement replay.
- [ ] Add `Navigator::StateDef_t = typename Filter_t::StateDef_t` and use it to consolidate repeated state-definition spelling inside Navigator.
- [ ] Normalize math helper names: use `quaternion_from_rpy_rad()` / `rpy_rad_from_quaternion()` for roll-pitch-yaw Euler conversions, and add `quaternion_from_rotvec_rad()` / `rotvec_rad_from_quaternion()` for small-angle or Rodrigues rotation-vector use cases. Keep nominal-state attitude and error-state perturbation vocabulary separate in names and tests.
- [ ] Decide and implement the migration path from the current RPY attitude segment to the `navigator_ecef_v1` nominal quaternion-state design. Until that migration is complete, document that the current ECEF propagation uses quaternion math internally but stores nominal attitude back into the existing RPY segment as a transitional state representation.
- [ ] Leave PVA sub-selection alone for now. Do not introduce PVA state views or copies unless a zero-runtime-cost view abstraction becomes clearly worthwhile.
- [ ] Standardize function signatures in Navigator/propagation code: input parameters first as `const&`, in/out parameters second as non-const references, and output-only parameters last. Add this rule to the agent/design guidance before applying broad cleanup.
- [ ] Add a tight INS diagnosis pass before expanding aiding: create an IMU-only stationary validation configuration or test harness, run ideal stationary truth with ideal IMU and no GNSS, and verify that pure strapdown propagation holds ECEF position/velocity/attitude within tight tolerances over a short run.
...
- [ ] Add runtime-configurable console/log output rates. Default the console heartbeat to 1 Hz and include time, position, velocity, and attitude; keep file logging rates separate from truth/IMU system rates so 1000 Hz simulation does not force 1000 Hz logs.
