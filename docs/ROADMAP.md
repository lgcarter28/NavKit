...
- [x] Rename current runtime initialization inputs away from filter-error language such as `initial_position_offset_m`. Prefer explicit initializer types such as `pva_error` for deterministic JSON-provided PVA error, `pva_random_error` for simulator-side random PVA error generated from covariance, and `pva_direct` for direct startup-message values.
...
- [x] Restructure runtime initialization JSON around explicit `pva_initialization` and `filter_initialization` sections. Use `pva_error` with frame/unit-bearing fields such as `p_e_m`, `v_e_mps`, and `rotvec_b2e_rad`, or local-level equivalents such as `p_n_m`, `v_n_mps`, and `rotvec_b2n_rad`; use `pva_random_error` with `pva_error_cov` containing either `diag` with 9 diagonal covariance values or `full` with 81 row-major covariance values; use `pva_direct` for direct PVA startup values. Drop sigma-based initialization inputs from this path.
- [x] Add runtime validation that rejects missing/ambiguous `pva_error_cov` shapes, wrong `diag`/`full` lengths, unsupported initializer types, and malformed PVA vectors with clear messages.
...
- [x] Add `pva_error_frame` for `pva_random_error` initialization so random covariance draws can be interpreted in ECEF or NED before conversion to the internal ECEF-resolved error convention.
...
## Pass 5e.9 — Advanced analysis/restart initialization
...
- [ ] Add focused tests demonstrating the separation: normal PVA/covariance startup remains unchanged, direct non-PVA nominal override initializes only selected state segments, Monte Carlo estimate-error initialization produces deterministic seeded draws, and transfer alignment remains independent.
