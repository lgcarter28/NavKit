...
- [x] Migrate from transitional RPY nominal attitude storage to the `navigator_ecef_v1` split-state design by adding `DefaultInsStateDef`: nominal filter state now stores ECEF-to-body attitude in `Quat`, while the covariance/error-state vector keeps the 3D small-angle `Att` perturbation.
