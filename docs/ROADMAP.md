...
- [x] Rename PVA fields for clarity and unit consistency, including staged `attitude_rad`/`att_rad` vocabulary to `rpy_be_rad`. Align covariance field names with the PVA state vocabulary and units while avoiding one-off 3x3 covariance block members that lose cross-correlation terms.
