...
- [ ] Replace dummy-object type dispatch such as `typename NavKit::MeasurementStatisticsTuple{}` with explicit type-level helper APIs, for example `log_measurement_statistics<typename NavKit::MeasurementStatisticsTuple>(logger, filter)`, so `SimulationApp` does not appear to pass empty runtime statistics state.
