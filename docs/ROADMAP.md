...
- [ ] Clean up concrete config naming so scenario identity comes from the file path and local namespace, not repeated type prefixes. For example, use an internal namespace such as `navkit::config::navkit::profiled_stationary_gnss` with role-based names like `NumericConfig`, `GnssBufferConfig`, and `ProductConfig`, then export a stable scenario-specific alias such as `navkit::config::navkit::ProfiledStationaryGnssConfig`.
