...
concept InjectionPolicy =
    requires(State<StateDef>& x, State<StateDef>& dx)
{
    T::apply(x, dx);
};
\end{lstlisting}
...
{
    static void apply(State<MyStateDef>& x, State<MyStateDef>& dx);
};
\end{lstlisting}
...
concept MeasurementPolicy =
    requires(State<StateDef>& x, typename T::NoiseContext& ctx)
{
    T::M;
    typename T::O_t;
    typename T::R_t;
...
Sensor and noise policies may also depend on measurement and state definition
policies.

For example:

\begin{lstlisting}[language=C++]
template<typename T, typename Measurement, StateDefPolicy StateDef>
concept NoisePolicy =
    MeasurementPolicy<Measurement, StateDef> &&
    requires(...)
{
    T::compute_r(...);
};
