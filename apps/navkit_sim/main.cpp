...
int main(int argc, char** argv)
{
    try {
        ...
        truth_csv << "time_s,p_e_x_m,p_e_y_m,p_e_z_m,v_e_x_mps,v_e_y_mps,v_e_z_mps,a_e_x_mps2,a_e_"
                     "y_mps2,a_e_z_mps2,q_eb_w,q_eb_x,q_eb_y,q_eb_z,w_ib_b_x_radps,w_ib_b_y_radps,"
                     "w_ib_b_z_radps\n";
        gnss_csv << "time_s,p_e_x_m,p_e_y_m,p_e_z_m\n";
        nav_csv << "time_s,p_e_x_m,p_e_y_m,p_e_z_m,err_p_e_x_m,err_p_e_y_m,err_p_e_z_m,sigma_p_e_x_"
                   "m,sigma_p_e_y_m,sigma_p_e_z_m\n";
        ...
        for (const auto& sample : truth) {
            truth_csv << sample.time << ',' << sample.p_e.x() << ',' << sample.p_e.y() << ','
                      << sample.p_e.z() << ',' << sample.v_e.x() << ',' << sample.v_e.y() << ','
                      << sample.v_e.z() << ',' << sample.a_e.x() << ',' << sample.a_e.y() << ','
                      << sample.a_e.z() << ',' << sample.q_eb.w() << ',' << sample.q_eb.x() << ','
                      << sample.q_eb.y() << ',' << sample.q_eb.z() << ',' << sample.w_ib_b.x()
                      << ',' << sample.w_ib_b.y() << ',' << sample.w_ib_b.z() << '\n';
            ...
            nav_csv << sample.time << ',' << p_est.x() << ',' << p_est.y() << ',' << p_est.z()
                    << ',' << err.x() << ',' << err.y() << ',' << err.z() << ','
        ...
        const json gnss_meta = {{"schema", "gnss_pos_v1"},
                                {"file", "gnss.csv"},
                                {"units", {{"time", "s"}, {"p_e", "m"}}},
                                {"noise",
                                 {{"sigma_h_m", gnss_cfg.sigma_h_m},
                                  {"sigma_v_m", gnss_cfg.sigma_v_m},
                                  {"seed", gnss_cfg.seed}}}};
        const json nav_meta = {
        ...
        write_json_file(output_dir / "run_manifest.json",
                        {{"run_name", run_name},
                         {"config", cfg},
                         {"logs",
