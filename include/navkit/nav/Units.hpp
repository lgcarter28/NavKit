...
template <typename Dimension, typename Ratio = std::ratio<1>, typename Scalar = navkit::Scalar_t>
struct Unit {
    using dim = Dimension;
    using ratio = Ratio;
    using scalar = Scalar;
    static constexpr Scalar scale_to_si = static_cast<Scalar>(Ratio::num) / static_cast<Scalar>(Ratio::den);
};
...
template <typename UnitT, typename Frame, int N, typename Scalar = navkit::Scalar_t>
struct Vec {
    using unit = UnitT;
...
auto operator+(const Vec<U1, Frame, N, Scalar>& a, const Vec<U2, Frame, N, Scalar>& b) {
    static_assert(std::is_same_v<typename U1::dim, typename U2::dim>, "Cannot add vectors with different dimensions");
...
auto unit_cast(const Vec<U1, Frame, N, Scalar>& x) {
    static_assert(std::is_same_v<typename U1::dim, typename U2::dim>, "Cannot cast between units with different dimensions");
