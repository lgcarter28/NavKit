...
public:
    ...
    [[nodiscard]] const Filter_t& filter() const { return m_filter; }

    Sensors_t& sensors() { return m_sensors; }
    [[nodiscard]] const Sensors_t& sensors() const { return m_sensors; }
    ...
    void process_measurements() {
        std::apply(
            [this](auto&... sensor_obj) {
                (process_one_sensor(sensor_obj), ...);
            },
            m_sensors);
