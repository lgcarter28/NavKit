...
def default_build_dir(root: Path, build_type: str, navkit_config: str) -> Path:
    """Derive the default build tree from build type and selected config header."""
    config_path = validate_navkit_config(navkit_config)
    return root / "build" / build_type / config_path.with_suffix("")


def resolve_build_dir(
    root: Path, build_type: str, navkit_config: str, build_dir_arg: Path | None
) -> Path:
    """Resolve an explicit build directory or the config-rooted default."""
    if build_dir_arg is None:
        return default_build_dir(root, build_type, navkit_config)
